import { canonicalRef, eventsFor } from './router';
...
describe('eventsFor', () => {
  ...
  // An unevaluated run is a statement about US. Whatever a row happens to hold,
  // a run that measured nothing cannot be the moment a medal was earned.
  it('never reads a medal off an unevaluated run', () => {
    const events = eventsFor([
      run({ kind: 'unevaluated', medal: 'gold' }),
      run({ runAt: '2026-08-31T00:00:00.000Z', kind: 'unevaluated', medal: null }),
    ]);
    expect(events.filter(e => e.type === 'medal-earned')).toEqual([]);
  });
...
describe('canonicalRef', () => {
  // The writer and the reader must agree on one spelling or they never meet:
  // a component swept under one and requested under the other is a permanent
  // 404 that reads as though the sweep never ran.
  it('lowercases kind, namespace and name', () => {
    expect(canonicalRef('Component:Default/MySite')).toBe(
      'component:default/mysite',
    );
  });

  it('fills in the default namespace', () => {
    expect(canonicalRef('component:site')).toBe('component:default/site');
  });

  it('leaves an already canonical reference alone', () => {
    expect(canonicalRef('component:default/site')).toBe(
      'component:default/site',
    );
  });

  it('throws on something that is not an entity reference', () => {
    expect(() => canonicalRef('')).toThrow();
  });
});
