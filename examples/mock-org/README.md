...
- **Four gildi** (`org.yaml`), two of each form from the design:
  - `security-gildi` — **aspect-aligned** (stewards the `security` aspect). Members: Astrid, Leif, and Foxholm's Dagny (cross-department).
  - `platform-gildi` — **aspect-aligned** (stewards `operational-readiness`).
  - `release-captains-gildi` — **craft-aligned** (stewards the `release-captain` craft). Members: Bjorn, Runa.
  - `incident-commanders-gildi` — **craft-aligned** (stewards the `incident-commander` craft).
- **Software graph** (`software.yaml` + per-repo catalog-info): systems `parcel-tracking`, `shipping`, and Foxholm's `returns-logistics`; components `tracking-web`, `shipping-orchestrator`, `carrier-gateway`, `label-service` (pristine and unenrolled — **the adoption demo target**), `refund-service`, `intake-scanner`, and the `prod-cluster` Resource. Two components live where real ones would: `tracking-api` and `security-practice` are declared in **their own repos' `catalog-info.yaml`** (`repos/tracking-api/`, `repos/security-aspect/`) — the live topology, registered directly here in place of provider discovery.
- **The practice and its aspect**: `security-practice` (`spec.type: practice`) is the *institution's* catalog face; `repos/security-aspect/` is its **aspect — the module**: the blocked standard, the paved road, the remediation vísar, and **two adoption doors** — `template.yaml` (scaffolder door, visible on the Create page) and `SKILL.md` (agent door). Entity links tie it all together one click from the catalog.
- **The instance itself** — the **non-fiction** corner of the catalog, cataloged as the System of parts it actually is: Domain `siliconsaga` → System `leidangr` → Component `gildi` (title "Guild Hall", `spec.type: plugin`). The Domain and System live in the root `catalog-info.yaml` so the System's TechDocs `dir:.` resolves; each cornerstone declares itself in its own package directory (`plugins/gildi/catalog-info.yaml`), the same live-topology shape `tracking-api` and `security-practice` use. The System's vísir is the demo/onboarding grand tour (`docs/demo-visir.md` — start there). Sibling cornerstones join the System as they appear, and any other instance adopting this model repeats the same hierarchy. It was once a single `guild-hall` Component of type `hub`, until that retired: a hub is a rendered page, not a domain noun.
...
**The update loop (why the drive exists):** the aspect registry (`guildhall/aspects.yaml`) pins the security module at **1.4**; each enrolled component records what it actually adopted in `siliconsaga.org/aspect-versions`, and past adoptions keep their PR in `siliconsaga.org/adoption-record` (both inert — shapes tentative until the scorecard engine consumes them; `.example` hosts mark the PR links as fiction). The gap between the registry version and a component's adopted version is what a drive closes: `shipping-orchestrator` (adopted May, 1.4, green), `refund-service` (adopted during Foxholm onboarding, 1.4, green — the reuse beat: same standard, same guild, zero new documents), `intake-scanner` (1.3, bronze — legacy build image), `carrier-gateway` (1.2, pre-paved-road, the oldest debt). Astrid's mid-run Saga narrates exactly this tally; `label-service` stays untouched as the live-adoption target.

## Guildhall files (mostly not catalog entities — future plugin input)

The `guildhall/` tree and most of `repos/` are **plain YAML/markdown, deliberately not ingested** — fixtures documenting the exact shapes design §5 assigns to vocabularies and Git-backed standards. Three per-repo files ARE intentionally ingested (registered in `app-config.yaml`, standing in for provider discovery): `repos/tracking-api/catalog-info.yaml`, `repos/security-aspect/catalog-info.yaml`, and `repos/security-aspect/template.yaml` (the adoption template — vanilla `Template` kind). Paths inside the fixtures are file-relative:
...
- `repos/security-aspect/` — the **aspect repo (the module)**: the security standard (**blocks** of trials by tool/sub-concern with **facet** applicability, tiers laddering across them), the paved road (`pipeline-templates/`), the two adoption doors (`template.yaml`, `SKILL.md`), teaching vísar (`docs/`), and a parameterized operational vísir (`docs/runbooks/`). Its `catalog-info.yaml` declares the practice Component.
