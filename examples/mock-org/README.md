...
- **The instance itself** (root `catalog-info.yaml` — moved there so its TechDocs `dir:.` resolves): `leidangr`, an ordinary `service` Component owned by `team-devex` — the one **non-fiction** entity in the seed: it is this very Backstage instance, and its vísir is the demo/onboarding grand tour (`docs/demo-visir.md` — start there). It was once `guild-hall`, type `hub`, until that concept retired: a hub is a rendered page, not a domain noun. The entity remains only because TechDocs needs a catalog entity to anchor to, which a plugin page can never be.
