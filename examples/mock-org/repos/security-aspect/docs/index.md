...
- **The adoption doors** — how the aspect gets applied: `template.yaml` (the Create-page door — a scaffolder Template) and `SKILL.md` (the agent door, for practitioners who never leave their CLI). Both read this same module.
...
Questions → `#security-gildi`. If your service isn't enrolled yet, run the adoption — or add `security` to its `siliconsaga.org/aspects` annotation by hand and start at bronze.
