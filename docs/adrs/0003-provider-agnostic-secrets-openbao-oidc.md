...
The local app needs a real secret (a Gitea token) from a central, multi-consumer OpenBao that may run on local k3s or on GKE. The app must not be coupled to the secret store, and the same app should run regardless of which OpenBao (or none) is reachable.
...
- Good: the same Backstage runs against homelab, GKE, or nothing; reproduces the "run one command, log in via browser, get a local secrets file" UX using infrastructure already run. No Consul — Backstage `$env`/`$file` replace runtime templating.
